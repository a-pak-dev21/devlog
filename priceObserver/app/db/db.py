...
def save_snapshot(records: list[dict], timestamp: datetime) -> dict[str, int] | None:
...
if __name__ == "__main__":
    ...
    result = run_snapshot(test_pairs)
    if result is None:
        logger.error("Function 'run_snapshot' returned None")
    else:
        records, snapshot_t = result
        save_snapshot(records=records, timestamp=snapshot_t)